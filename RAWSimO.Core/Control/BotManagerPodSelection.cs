...
namespace RAWSimO.Core.Control
{
    ...
    public abstract partial class BotManager
    {
        ...
        public HashSet<Order> SelectedOrder = new HashSet<Order>();
        /// <summary>
        ...
        public double Score(PCScorerPodForOStationBotCompleteable config, Bot bot, Pod pod, OutputStation station)
        {
            ...
            foreach (var order in station.AssignedOrders.Where(v=>!SelectedOrder.Contains(v)))
            {
            ...
            //        foreach (var itemDemand in itemDemands)
            //        {
            //            if(_availableCounts[itemDemand.Key] >= itemDemand.Count())
            //            {
            //                _availableCounts[itemDemand.Key] -= itemDemand.Count();
            //                // Update number of completeable orders
            //                completeableitems += itemDemand.Count();
            //            }
            //            else
            //            {
            //                // Update number of completeable orders
            //                completeableitems += _availableCounts[itemDemand.Key];
            //                _availableCounts[itemDemand.Key] -= 0;
            //            }
            //        }
            //    }
            //}
            return -(completeableAssignedOrders);
        }
        /// <summary>
        /// Determines a score that can be used to decide about an assignment.
        /// </summary>
        ...
        protected bool DoExtractTaskForStation2(Bot bot, OutputStation oStation, bool extendSearch)
        {
            ...
            if (bot.Pod != null)
            {
                ...
                else
                {
                    ...
                    if (Instance.ControllerConfig.PodStorageConfig is GM_1PodStorageConfiguration)
                    {
                        if (Instance.InputStations.Where(v => v.X == bot.Pod.X && v.Y == bot.Pod.Y).Count() > 0)
                            EnqueueParkPod(bot, bot.Pod, Instance.Controller.PodStorageManager.GetStorageLocation(bot.Pod));
                        else
                        {
                            // Pod is not useful anymore - put it away   将pod送回存储区域
                            EnqueueParkPod(bot, bot.Pod, Instance.Controller.PodStorageManager.GetStorageLocation1(bot.Pod));
                        }
                    }
                    else
                        EnqueueParkPod(bot, bot.Pod, Instance.Controller.PodStorageManager.GetStorageLocation(bot.Pod));
            ...
            else
            {
                ...
                {
                    foreach (var station in bot.Tier.OutputStations.Where(s => Instance.ResourceManager._Ziops1[s].ContainsKey(Instance.ResourceManager.BottoPod[bot])))
                        oStation = station;
                ...
                {
                    bestPod = Instance.ResourceManager._Ziops1[oStation].Select(s => s.Key).Where(v => Instance.ResourceManager.UnusedPods.Contains(v) && !Instance.ResourceManager.BottoPod.ContainsValue(v)).OrderBy(v => Distances.CalculateShortestPathPodSafe
        ...
        /// <returns></returns>
        protected bool CreateTask(Bot bot,DefaultPodSelectionConfiguration config)
        {
            HashSet<Pod> SelectedPod = new HashSet<Pod>();
            foreach (var station in bot.Tier.OutputStations)
            {
                foreach (var pod in Instance.ResourceManager._Ziops1[station].Keys)
                    SelectedPod.Add(pod);
            }
            if (SelectedPod.Count() > 5) 
                return false;
            //foreach (var station in bot.Tier.OutputStations)
            //    Instance.ResourceManager._Ziops1[station].Clear();
            foreach (var station in bot.Tier.OutputStations.Where(s => Instance.ResourceManager.GetExtractRequestsOfStation(s).Count() > 0))
            {
                SelectedOrder.Clear();
                L: _bestPodOStationCandidateSelector.Recycle();
                // Determine best pod
                Pod bestPod = null;
                OutputStation beststation = null;
                foreach (var pod in Instance.ResourceManager.UnusedPods.Where(p => AnyRelevantRequests(p, station, 
                    config.FilterForConsideration) && !Instance.ResourceManager.BottoPod.ContainsValue(p) && !SelectedPod.Contains(p)))
                {
                    // Update current candidate to assess
                    _currentBot = bot;
                ...
                {
                    // Get all fitting requests
                    List<ExtractRequest> fittingRequests = GetPossibleRequests(bestPod, beststation, config.FilterForReservation);
                    foreach (var Request in fittingRequests)
                    {
                        Instance.ResourceManager.RemoveavailableExtractRequestsPerStation(Request);
                        SelectedOrder.Add(Request.Order);
                    }
                    // Log
                    Instance.ResourceManager._availablePodsPerStation[beststation].Add(bestPod);
                    Instance.ResourceManager._Ziops1[beststation].Add(bestPod, fittingRequests);
                    SelectedPod.Add(bestPod);
                    if (SelectedOrder.Count() > 1)
                        continue;
                    goto L;
                }
                if (SelectedPod.Count() > 20)
                    return false;
            }
            return false;
        }
        ...
        /// <param name="config">Pod selection config to use.</param>
        /// /// <returns>true if bot has a new extract task or is just parking the pod, false if it is doing a rest task.</returns>
        protected bool DoExtractTaskForStation3(Bot bot, OutputStation oStation, bool extendSearch, double extendedSearchRadius, DefaultPodSelectionConfiguration config)
        {
            // Init
            InitPodSelection();
            // --> Prepare best candidate selectors
            ...
            else
            {
                ...
                else
                {
                    ...
                    foreach (var pod in Instance.ResourceManager.UnusedPods
                         ...
                         !Instance.ResourceManager.BottoPod.ContainsValue(p) && !SelectedPod.Contains(p)))
                    {
                        // Update current candidate to assess
                        _currentBot = bot;
                ...
                if (bestPod != null)
                {
                    List<ExtractRequest> fittingRequests = new List<ExtractRequest>();
                    if (ll==1)
                    {
                        Instance.ResourceManager._availablePodsPerStation[oStation].Remove(bestPod);
                        // Get all fitting requests
                        fittingRequests = Instance.ResourceManager._Ziops1[oStation][bestPod];
                        Instance.ResourceManager._Ziops1[oStation].Remove(bestPod);
                    }
                    else
                    {
                        // Get all fitting requests
                        fittingRequests = GetPossibleRequests(bestPod, oStation, config.FilterForReservation);
                    }
                    if (fittingRequests.Count == 0)
                        return false;
                    // Log
