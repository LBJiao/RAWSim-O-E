        ...
        public void HeuristicsPOAandPPS(Func<OutputStation, bool> validStationNormalAssignment, bool CsOd)
        {
            ...
            while (furtherOptions)
            {
                ...
                L:
                    ...
                    {
                        _bestCandidateSelectNormal.Recycle();//初始化_currentValue和_bestScores
                        Order chosenOrder = null;
                        // Search for best order for the station in all fulfillable orders        选择的订单必须满足已分配的订单的库存的数量约束
                        ...
                        if (chosenOrder != null)
                        {
                            ...
                            IEnumerable<ExtractRequest> itemDemands = Instance.ResourceManager.GetExtractRequestsOfOrder(chosenOrder);//要标记的item的集合
                            int i = 0;
                            // Get current content of the pod
                            if (BestPodsbyS.Count == 0)
                            {
                                foreach (var pod in Ns[chosenStation].OrderBy(v => v.Value).Select(s => s.Key))//按排好的顺序对pod中的item进行标记
                                {
                                    if (itemDemands.Where(v => !requestsToHandleofAll.Contains(v)).Any(g => pod.IsAvailable(g.Item)))
                                    {
                                        ...
                                        if (Instance.ResourceManager._Ziops1[chosenStation].ContainsKey(pod))//对Ziops1进行更新
                    ...
                    {
                        _bestPodOStationCandidateSelector.Recycle();//初始化_currentValue和_bestScores
                        Pod BestPod = null;
                        ...
                        goto L;//经过pps已经分配了一个pod,下一步返回POA进行任务分配
                    }
