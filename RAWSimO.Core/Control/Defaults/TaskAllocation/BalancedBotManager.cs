...
namespace RAWSimO.Core.Control.Defaults.TaskAllocation
{
    ...
    public class BalancedBotManager : BotManager
    {
        ...
        private void ReallocateBots(bool overrideActivity)
        {
            ...
            //最少保留一个inputstation
            //if(activeInputStations.Count()==0)
            //{
            //    Random rd = new Random(Guid.NewGuid().GetHashCode());
            //    activeInputStations = Instance.InputStations.Where(v => v.ID == rd.Next(1, Instance.InputStations.Count())).ToList();
            //}
            ...
            bool bb = true;
            if (Instance.ControllerConfig.OrderBatchingConfig is GM1Configuration || Instance.ControllerConfig.OrderBatchingConfig is GM2Configuration)
            {
            ...
            {
                if (Instance.ResourceManager.BottoPod.Count() == 2)
                    System.Threading.Thread.Sleep(1);
                Instance._outputstationbots.Add(bot1.Key);
                _stationBots[bot1.Value].Remove(bot1.Key);
                if (Instance.ControllerConfig.OrderBatchingConfig is GM1Configuration)
                {
                    bool ifbreak = false;
