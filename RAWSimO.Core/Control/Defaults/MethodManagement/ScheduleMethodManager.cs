            ...
            if (_podStorageManagerQueue.Any() && currentTime > TranslateTime(_podStorageManagerQueue.First().Key))
            {
                ...
                switch (nextPodManagerType)
                {
                    case PodStorageMethodType.Nearest:
                        ...
                        break;
                    case PodStorageMethodType.GM_1:
                        {
                            Instance.ControllerConfig.PodStorageConfig = new GM_1PodStorageConfiguration();
                            newPodStorageManager = new GM_1PodStorageManager(Instance);
                        }
                        break;
