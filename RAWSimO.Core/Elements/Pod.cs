...
namespace RAWSimO.Core.Elements
{
    ...
    public class Pod : Circle, IPodInfo, IExposeVolatileID
    {
        ...
        { 
            if(itemDescriptions.Where(v=> _itemDescriptionCountAvailable != null && _itemDescriptionCountAvailable[v] > 0).Count()>0)
                return true;
            //foreach (var item in itemDescriptions)
            //{
            //    if (_itemDescriptionCountAvailable != null && _itemDescriptionCountAvailable[item] > 0)
            //        return true;
            //}    
            return false;
        }
